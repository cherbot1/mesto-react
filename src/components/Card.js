...
function Card(props) {
    ...
    return (
            <li className="element">
                <button type="button" className="element__delete-button" aria-label="Удалить"
                        title="Удалить"></button>
                <figure className="element__figure">
                    <div className="element__image-wrapper">
                        <img className="element__image" src={props.cardInfo.link} alt={props.cardInfo.name} onClick={handleClick}></img>
                    </div>
                    <figcaption className="element__figcaption">
                        <h2 className="element__figcaption-text">{props.cardInfo.name}</h2>
                        <div className="element__like-section">
                            <button type="button" className="element__like-button" aria-label="Нравится"
